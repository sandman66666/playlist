...
async def add_tracks_to_playlist(
    ...
    """
    try:
        # First, check if the user has permission to modify this playlist
        playlist = sp.playlist(playlist_id)
        current_user = sp.me()
        
        # Check if user owns the playlist or if it's collaborative
        if not (playlist['owner']['id'] == current_user['id'] or playlist.get('collaborative', False)):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to modify this playlist. You must be the owner or the playlist must be collaborative."
            )
